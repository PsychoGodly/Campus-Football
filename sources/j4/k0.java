package j4;

import android.content.Context;
import android.content.res.AssetFileDescriptor;
import android.content.res.Resources;
import android.net.Uri;
import android.text.TextUtils;
import com.applovin.mediation.MaxReward;
import com.facebook.ads.AdError;
import com.unity3d.services.UnityAdsConstants;
import java.io.EOFException;
import java.io.FileInputStream;
import java.io.IOException;
import java.io.InputStream;
import java.nio.channels.FileChannel;
import k4.n0;

/* compiled from: RawResourceDataSource */
public final class k0 extends g {

    /* renamed from: e  reason: collision with root package name */
    private final Resources f19357e;

    /* renamed from: f  reason: collision with root package name */
    private final String f19358f;

    /* renamed from: g  reason: collision with root package name */
    private Uri f19359g;

    /* renamed from: h  reason: collision with root package name */
    private AssetFileDescriptor f19360h;

    /* renamed from: i  reason: collision with root package name */
    private InputStream f19361i;

    /* renamed from: j  reason: collision with root package name */
    private long f19362j;

    /* renamed from: k  reason: collision with root package name */
    private boolean f19363k;

    /* compiled from: RawResourceDataSource */
    public static class a extends m {
        public a(String str, Throwable th, int i10) {
            super(str, th, i10);
        }
    }

    public k0(Context context) {
        super(false);
        this.f19357e = context.getResources();
        this.f19358f = context.getPackageName();
    }

    public static Uri buildRawResourceUri(int i10) {
        return Uri.parse("rawresource:///" + i10);
    }

    public void close() throws a {
        this.f19359g = null;
        try {
            InputStream inputStream = this.f19361i;
            if (inputStream != null) {
                inputStream.close();
            }
            this.f19361i = null;
            try {
                AssetFileDescriptor assetFileDescriptor = this.f19360h;
                if (assetFileDescriptor != null) {
                    assetFileDescriptor.close();
                }
                this.f19360h = null;
                if (this.f19363k) {
                    this.f19363k = false;
                    r();
                }
            } catch (IOException e10) {
                throw new a((String) null, e10, AdError.SERVER_ERROR_CODE);
            } catch (Throwable th) {
                this.f19360h = null;
                if (this.f19363k) {
                    this.f19363k = false;
                    r();
                }
                throw th;
            }
        } catch (IOException e11) {
            throw new a((String) null, e11, AdError.SERVER_ERROR_CODE);
        } catch (Throwable th2) {
            this.f19361i = null;
            try {
                AssetFileDescriptor assetFileDescriptor2 = this.f19360h;
                if (assetFileDescriptor2 != null) {
                    assetFileDescriptor2.close();
                }
                this.f19360h = null;
                if (this.f19363k) {
                    this.f19363k = false;
                    r();
                }
                throw th2;
            } catch (IOException e12) {
                throw new a((String) null, e12, AdError.SERVER_ERROR_CODE);
            } catch (Throwable th3) {
                this.f19360h = null;
                if (this.f19363k) {
                    this.f19363k = false;
                    r();
                }
                throw th3;
            }
        }
    }

    public long f(p pVar) throws a {
        int i10;
        String str;
        p pVar2 = pVar;
        Uri uri = pVar2.f19387a;
        this.f19359g = uri;
        if (TextUtils.equals("rawresource", uri.getScheme()) || (TextUtils.equals("android.resource", uri.getScheme()) && uri.getPathSegments().size() == 1 && ((String) k4.a.e(uri.getLastPathSegment())).matches("\\d+"))) {
            try {
                i10 = Integer.parseInt((String) k4.a.e(uri.getLastPathSegment()));
            } catch (NumberFormatException unused) {
                throw new a("Resource identifier must be an integer.", (Throwable) null, 1004);
            }
        } else if (TextUtils.equals("android.resource", uri.getScheme())) {
            String str2 = (String) k4.a.e(uri.getPath());
            if (str2.startsWith(UnityAdsConstants.DefaultUrls.AD_ASSET_PATH)) {
                str2 = str2.substring(1);
            }
            String host = uri.getHost();
            StringBuilder sb2 = new StringBuilder();
            if (TextUtils.isEmpty(host)) {
                str = MaxReward.DEFAULT_LABEL;
            } else {
                str = host + ":";
            }
            sb2.append(str);
            sb2.append(str2);
            i10 = this.f19357e.getIdentifier(sb2.toString(), "raw", this.f19358f);
            if (i10 == 0) {
                throw new a("Resource not found.", (Throwable) null, 2005);
            }
        } else {
            throw new a("URI must either use scheme rawresource or android.resource", (Throwable) null, 1004);
        }
        s(pVar);
        try {
            AssetFileDescriptor openRawResourceFd = this.f19357e.openRawResourceFd(i10);
            this.f19360h = openRawResourceFd;
            if (openRawResourceFd != null) {
                long length = openRawResourceFd.getLength();
                FileInputStream fileInputStream = new FileInputStream(openRawResourceFd.getFileDescriptor());
                this.f19361i = fileInputStream;
                int i11 = (length > -1 ? 1 : (length == -1 ? 0 : -1));
                if (i11 != 0) {
                    try {
                        if (pVar2.f19393g > length) {
                            throw new a((String) null, (Throwable) null, AdError.REMOTE_ADS_SERVICE_ERROR);
                        }
                    } catch (a e10) {
                        throw e10;
                    } catch (IOException e11) {
                        throw new a((String) null, e11, AdError.SERVER_ERROR_CODE);
                    }
                }
                long startOffset = openRawResourceFd.getStartOffset();
                long skip = fileInputStream.skip(pVar2.f19393g + startOffset) - startOffset;
                if (skip == pVar2.f19393g) {
                    if (i11 == 0) {
                        FileChannel channel = fileInputStream.getChannel();
                        if (channel.size() == 0) {
                            this.f19362j = -1;
                        } else {
                            long size = channel.size() - channel.position();
                            this.f19362j = size;
                            if (size < 0) {
                                throw new a((String) null, (Throwable) null, AdError.REMOTE_ADS_SERVICE_ERROR);
                            }
                        }
                    } else {
                        long j10 = length - skip;
                        this.f19362j = j10;
                        if (j10 < 0) {
                            throw new m(AdError.REMOTE_ADS_SERVICE_ERROR);
                        }
                    }
                    long j11 = pVar2.f19394h;
                    if (j11 != -1) {
                        long j12 = this.f19362j;
                        if (j12 != -1) {
                            j11 = Math.min(j12, j11);
                        }
                        this.f19362j = j11;
                    }
                    this.f19363k = true;
                    t(pVar);
                    long j13 = pVar2.f19394h;
                    return j13 != -1 ? j13 : this.f19362j;
                }
                throw new a((String) null, (Throwable) null, AdError.REMOTE_ADS_SERVICE_ERROR);
            }
            throw new a("Resource is compressed: " + uri, (Throwable) null, AdError.SERVER_ERROR_CODE);
        } catch (Resources.NotFoundException e12) {
            throw new a((String) null, e12, 2005);
        }
    }

    public Uri o() {
        return this.f19359g;
    }

    public int read(byte[] bArr, int i10, int i11) throws a {
        if (i11 == 0) {
            return 0;
        }
        long j10 = this.f19362j;
        if (j10 == 0) {
            return -1;
        }
        if (j10 != -1) {
            try {
                i11 = (int) Math.min(j10, (long) i11);
            } catch (IOException e10) {
                throw new a((String) null, e10, AdError.SERVER_ERROR_CODE);
            }
        }
        int read = ((InputStream) n0.j(this.f19361i)).read(bArr, i10, i11);
        if (read != -1) {
            long j11 = this.f19362j;
            if (j11 != -1) {
                this.f19362j = j11 - ((long) read);
            }
            q(read);
            return read;
        } else if (this.f19362j == -1) {
            return -1;
        } else {
            throw new a("End of stream reached having not read sufficient data.", new EOFException(), AdError.SERVER_ERROR_CODE);
        }
    }
}
